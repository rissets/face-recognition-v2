...
class FaceAuthWebSocketClient:
    ...
    def create_authentication_session(self, user_id: str = None, require_liveness: bool = True) -> dict:
    ...
    async def handle_message(self, data: dict):
        ...
        if msg_type == "connection_established":
            ...
            print(f"   ⚠️  Please remove obstacles and try again")
            ...
            if data.get("success"):
                ...
                print(f"\n📊 Frame {data.get('frames_processed')}/{data.get('target_samples', data.get('min_frames_required', '?'))}")
                print(f"   Liveness Score: {data.get('liveness_score', 0):.2f}")
                print(f"   Blinks: {data.get('blinks_detected', 0)}/{data.get('blinks_required', 1)} {'✅' if blinks_ok else '❌'}")
                print(f"   Motion: {data.get('motion_events', 0)}/{data.get('motion_required', 1)} {'✅' if motion_ok else '❌'} (score: {data.get('motion_score', 0):.2f})")
                print(f"   No Obstacles: {'✅' if no_obstacles else '❌'}")
                print(f"   Quality: {data.get('quality_score', 0):.2f} (threshold: {data.get('quality_threshold', 0.65):.2f})")
                ...
                if liveness_ind:
                    print(f"   👁️  EAR: {liveness_ind.get('ear', 0):.3f} | Blink: {'✓' if liveness_ind.get('blink_verified') else '✗'}")
            ...
            if data.get('authenticated'):
                ...
                if encrypted_payload:
                    ...
                    if decrypted:
                        ...
                        print(f"      - Timestamp: {decrypted.get('timestamp')}")
                        print(f"      - Session Type: {decrypted.get('session_type')}")
                        print(f"      - Confidence: {decrypted.get('confidence', 0):.2%}")
                    else:
                        print("   ⚠️  Failed to decrypt response")
            print("="*60)
    ...
    def _draw_overlays(self, frame):
        ...
        try:
            ...
            if liveness_ind:
                ...
                # Frame counter
                cv2.putText(frame, f"Frame: {frame_counter}", (10, y_offset), 
                ...
                cv2.putText(frame, f"{blink_icon} Blinks: {blinks}", (10, y_offset), 
                ...
                cv2.rectangle(frame, (150, y_offset - 10), (150 + bar_length, y_offset + 5), ear_color, -1)
                y_offset += 25
                
                # Motion status with icon
                motion_color = (0, 255, 0) if motion_verified else (0, 165, 255)
                motion_icon = "[OK]" if motion_verified else "[??]"
                motion_text = f"OK ({motion_events})" if motion_verified else "Move head"
                cv2.putText(frame, f"{motion_icon} Motion: {motion_text}", 
                           (10, y_offset), cv2.FONT_HERSHEY_SIMPLEX, 0.6, motion_color, 2)
                y_offset += 25
            ...
            if response.get('quality_score') is not None:
                ...
                quality_color = (0, 255, 0) if quality >= quality_threshold else (0, 165, 255)
                quality_icon = "[OK]" if quality >= quality_threshold else "[!!]"
                cv2.putText(frame, f"{quality_icon} Quality: {quality:.2f}", (10, y_offset),
                           cv2.FONT_HERSHEY_SIMPLEX, 0.6, quality_color, 2)
                # Draw quality bar
                bar_length = int(quality * 100)
                cv2.rectangle(frame, (150, y_offset - 10), (150 + bar_length, y_offset + 5), quality_color, -1)
                y_offset += 25
            ...
            if response.get('liveness_verified') is not None:
                ...
                cv2.putText(frame, f"{liveness_icon} Liveness: {'VERIFIED' if liveness_verified else 'NOT OK'}", 
                           (10, y_offset), cv2.FONT_HERSHEY_SIMPLEX, 0.6, liveness_color, 2)
                y_offset += 25
            ...
            if obstacles:
                ...
                banner_height = 60
                overlay = frame.copy()
                cv2.rectangle(overlay, (0, h//2 - banner_height//2), (w, h//2 + banner_height//2), (0, 0, 255), -1)
                cv2.addWeighted(overlay, 0.7, frame, 0.3, 0, frame)
                
                # Draw border
                cv2.rectangle(frame, (0, h//2 - banner_height//2), (w, h//2 + banner_height//2), (0, 0, 255), 3)
                ...
                (text_w2, text_h2), _ = cv2.getTextSize(obstacle_text, cv2.FONT_HERSHEY_SIMPLEX, 0.7, 2)
                cv2.putText(frame, obstacle_text, 
                           (w//2 - text_w2//2, h//2 + 20), 
                           cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
                
                # Also show in panel
                y_offset += 5
                cv2.putText(frame, "OBSTACLES:", 
                           (10, y_offset), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 255), 2)
                y_offset += 20
                for obs in obstacles:
                    obstacle_conf = response.get('obstacle_confidence', {})
                    conf = obstacle_conf.get(obs, 0)
                    cv2.putText(frame, f"  {obs}: {conf:.2f}", 
            ...
            if message:
                ...
                max_width = w - 20
                words = message.split(' | ')  # Split by pipe for multi-part messages
                
                # Draw semi-transparent background for message
                msg_height = len(words) * 22 + 10
                overlay = frame.copy()
                cv2.rectangle(overlay, (5, h - msg_height - 5), (w - 5, h - 5), (0, 0, 0), -1)
                cv2.addWeighted(overlay, 0.7, frame, 0.3, 0, frame)
                
                # Draw message lines at bottom
                msg_y = h - msg_height + 5
                for line in words:
                    cv2.putText(frame, line, (10, msg_y),
    ...
    async def run_authentication(self, user_id: str = None, camera_index: int = 0):
        ...
        print(f"🔐 Starting {auth_type.lower()}" + (f" for user: {user_id}" if user_id else "") + "\n")
...
async def main():
    """Main function"""
    if len(sys.argv) < 4:
        print("Usage: python test_websocket_auth.py <API_KEY> <SECRET_KEY> <BASE_URL> [enrollment|authentication] [user_id]")
        print("\nExamples:")
        print("  # Enrollment")
        print("  python test_websocket_auth.py YOUR_API_KEY YOUR_SECRET_KEY http://localhost:8000 enrollment user123")
        print("\n  # Verification (with user_id)")
        print("  python test_websocket_auth.py YOUR_API_KEY YOUR_SECRET_KEY http://localhost:8000 authentication user123")
        print("\n  # Identification (without user_id)")
        print("  python test_websocket_auth.py YOUR_API_KEY YOUR_SECRET_KEY http://localhost:8000 authentication")
        sys.exit(1)
